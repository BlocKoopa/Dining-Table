				...
				if(((Engine.engine.getGameAttribute("QuestionMenuLv1") : Bool) == true))
				{
					if(((Engine.engine.getGameAttribute("Answered4") : Bool) == true))
					{
						runLater(1000 * 2, function(timeTask:TimedTask):Void
						{
							...
							actor.moveTo(325, 150, 0.5, Easing.linear);
				...
				if((((Engine.engine.getGameAttribute("Answered1") : Bool) == false) && ((Engine.engine.getGameAttribute("Answered3") : Bool) == false)))
				{
					if(((Engine.engine.getGameAttribute("Correct_2") : Bool) == false))
					{
						if(((Engine.engine.getGameAttribute("QuestionMenuLv1") : Bool) == false))
						{
							...
							{
								createRecycledActor(getActorType(36), 800, 50, Script.FRONT);
								runLater(1000 * 0.5, function(timeTask:TimedTask):Void
								{
									createRecycledActor(getActorType(38), 800, 150, Script.FRONT);
									runLater(1000 * 0.5, function(timeTask:TimedTask):Void
									{
										createRecycledActor(getActorType(42), 800, 250, Script.FRONT);
										runLater(1000 * 0.5, function(timeTask:TimedTask):Void
										{
											createRecycledActor(getActorType(40), 800, 350, Script.FRONT);
										}, actor);
									}, actor);
								}, actor);
							}, actor);
