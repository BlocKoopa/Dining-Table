	...
	override public function init()
	{
		...
		addMouseOverActorListener(actor, function(mouseState:Int, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled && 1 == mouseState)
			{
				...
				{
					if(((Engine.engine.getGameAttribute("SampleQuestionMenu") : Bool) == false))
					{
		...
		addMouseOverActorListener(actor, function(mouseState:Int, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled && 3 == mouseState)
			{
				if((((Engine.engine.getGameAttribute("Answered1") : Bool) == false) && ((Engine.engine.getGameAttribute("Answered2") : Bool) == false)))
				{
					if(((Engine.engine.getGameAttribute("SampleQuestionMenu") : Bool) == false))
					{
						Engine.engine.setGameAttribute("SampleQuestionMenu", true);
						actor.growTo(100/100, 100/100, 0, Easing.linear);
						actor.clearFilters();
		...
		addWhenUpdatedListener(null, function(elapsedTime:Float, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				if(((Engine.engine.getGameAttribute("SampleQuestionMenu") : Bool) == true))
				{
					if(((Engine.engine.getGameAttribute("Answered1") : Bool) == true))
					{
						Engine.engine.setGameAttribute("SampleQuestionMenu", false);
