...
class ActorEvents_12 extends ActorScript
{
	...
	override public function init()
	{
			...
			if(wrapper.enabled && -1 == mouseState)
			{
				if((((Engine.engine.getGameAttribute("Answered1") : Bool) == false) && ((Engine.engine.getGameAttribute("Answered2") : Bool) == false)))
				{
					if(((Engine.engine.getGameAttribute("QuestionMenuLv1") : Bool) == false))
					{
						if(((Engine.engine.getGameAttribute("Correct_1") : Bool) == false))
						{
							actor.growTo(40/100, 40/100, 0, Easing.linear);
							actor.clearFilters();
			...
			if(wrapper.enabled)
			{
				if(((Engine.engine.getGameAttribute("QuestionMenuLv1") : Bool) == true))
				{
					if(((Engine.engine.getGameAttribute("Answered1") : Bool) == true))
					{
						...
						{
							actor.sendToBack();
							actor.moveTo(400, 130, 0.5, Easing.linear);
							actor.growTo(40/100, 40/100, 0.5, Easing.linear);
					...
					if(((Engine.engine.getGameAttribute("Answered2") : Bool) == true))
					{
						Engine.engine.setGameAttribute("QuestionMenuLv1", false);
						runLater(1000 * 2, function(timeTask:TimedTask):Void
						{
					...
					if(((Engine.engine.getGameAttribute("Answered3") : Bool) == true))
					{
						Engine.engine.setGameAttribute("QuestionMenuLv1", false);
						Engine.engine.setGameAttribute("Correct_1", true);
						runLater(1000 * 2, function(timeTask:TimedTask):Void
						{
							actor.sendToBack();
							actor.alpha = 50 / 100;
		...
		addWhenUpdatedListener(null, function(elapsedTime:Float, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				if(((Engine.engine.getGameAttribute("PlateClick") : Bool) == false))
				{
					if(((Engine.engine.getGameAttribute("Correct_1") : Bool) == false))
					{
						actor.growTo(40/100, 40/100, 0, Easing.linear);
				...
				else if(actor.isMouseOver())
				{
					if(((Engine.engine.getGameAttribute("PlateHoverCheck") : Bool) == false))
					{
						actor.growTo(50/100, 50/100, 0, Easing.linear);
						actor.setFilter([createTintFilter(Utils.getColorRGB(255,255,51), 50/100)]);
						Engine.engine.setGameAttribute("PlateHoverCheck", true);
					}
				}
			}
		});
