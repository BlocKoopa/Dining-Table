...
class ActorEvents_44 extends ActorScript
{
	...
	override public function init()
	{
		...
		addMouseOverActorListener(actor, function(mouseState:Int, list:Array<Dynamic>):Void
		{
			...
			{
				if((((Engine.engine.getGameAttribute("Answered7") : Bool) == false) && ((Engine.engine.getGameAttribute("Answered9") : Bool) == false)))
				{
					if(((Engine.engine.getGameAttribute("QuestionMenuLv1") : Bool) == false))
					{
						if(((Engine.engine.getGameAttribute("Correct_3") : Bool) == false))
						{
			...
			if(wrapper.enabled && 3 == mouseState)
			{
				if((((Engine.engine.getGameAttribute("Answered7") : Bool) == false) && ((Engine.engine.getGameAttribute("Answered9") : Bool) == false)))
				{
					if(((Engine.engine.getGameAttribute("QuestionMenuLv1") : Bool) == false))
					{
						...
						{
							createRecycledActor(getActorType(46), 800, 50, Script.FRONT);
							runLater(1000 * 0.5, function(timeTask:TimedTask):Void
							{
								createRecycledActor(getActorType(48), 800, 150, Script.FRONT);
								runLater(1000 * 0.5, function(timeTask:TimedTask):Void
								{
									createRecycledActor(getActorType(50), 800, 250, Script.FRONT);
									runLater(1000 * 0.5, function(timeTask:TimedTask):Void
									{
										createRecycledActor(getActorType(52), 800, 350, Script.FRONT);
									}, actor);
								}, actor);
							}, actor);
						}, actor);
