...
class ActorEvents_213 extends ActorScript
{
	...
	override public function init()
	{
		...
		addMouseOverActorListener(actor, function(mouseState:Int, list:Array<Dynamic>):Void
		{
			...
			{
				if((((Engine.engine.getGameAttribute("Answered23") : Bool) == false) && ((Engine.engine.getGameAttribute("Answered24") : Bool) == false)))
				{
					if(((Engine.engine.getGameAttribute("QuestionMenuLv1") : Bool) == false))
					{
						if(((Engine.engine.getGameAttribute("Correct_7") : Bool) == false))
						{
							actor.growTo(60/100, 60/100, 0, Easing.linear);
			...
			if(wrapper.enabled && -1 == mouseState)
			{
				if((((Engine.engine.getGameAttribute("Answered23") : Bool) == false) && ((Engine.engine.getGameAttribute("Answered24") : Bool) == false)))
				{
					if(((Engine.engine.getGameAttribute("QuestionMenuLv1") : Bool) == false))
					{
						if(((Engine.engine.getGameAttribute("Correct_7") : Bool) == false))
						{
							actor.growTo(50/100, 50/100, 0, Easing.linear);
							actor.clearFilters();
						}
					}
				}
				...
				if(((Engine.engine.getGameAttribute("QuestionMenuLv1") : Bool) == true))
				{
					if(((Engine.engine.getGameAttribute("Answered22") : Bool) == true))
					{
						runLater(1000 * 2, function(timeTask:TimedTask):Void
						{
							actor.sendToBack();
							actor.alpha = 50 / 100;
							actor.growTo(110/100, 110/100, 0.5, Easing.linear);
							actor.moveTo(620, 185, 0.5, Easing.linear);
							runLater(1000 * 0.5, function(timeTask:TimedTask):Void
							{
								Engine.engine.setGameAttribute("QuestionMenuLv1", false);
								Engine.engine.setGameAttribute("Correct_7", true);
					...
					else if(((Engine.engine.getGameAttribute("Answered23") : Bool) == true))
					{
						...
						{
							actor.sendToBack();
							actor.growTo(50/100, 50/100, 0.5, Easing.linear);
							actor.moveTo(86, -82, 0.5, Easing.linear);
							runLater(1000 * 0.5, function(timeTask:TimedTask):Void
							{
								Engine.engine.setGameAttribute("QuestionMenuLv1", false);
				...
				if((((Engine.engine.getGameAttribute("Answered23") : Bool) == false) && ((Engine.engine.getGameAttribute("Answered24") : Bool) == false)))
				{
					if(((Engine.engine.getGameAttribute("Correct_7") : Bool) == false))
					{
						if(((Engine.engine.getGameAttribute("QuestionMenuLv1") : Bool) == false))
						{
							...
							actor.clearFilters();
							actor.moveTo(30, 150, 0.5, Easing.linear);
							createRecycledActor(getActorType(9), 0, 0, Script.MIDDLE);
							getLastCreatedActor().alpha = 10 / 100;
							runLater(1000 * 1, function(timeTask:TimedTask):Void
							{
								createRecycledActor(getActorType(215), 800, 50, Script.FRONT);
								runLater(1000 * 0.5, function(timeTask:TimedTask):Void
								{
									createRecycledActor(getActorType(217), 800, 150, Script.FRONT);
									runLater(1000 * 0.5, function(timeTask:TimedTask):Void
									{
										createRecycledActor(getActorType(219), 800, 250, Script.FRONT);
										runLater(1000 * 0.5, function(timeTask:TimedTask):Void
										{
											createRecycledActor(getActorType(221), 800, 350, Script.FRONT);
										}, actor);
									}, actor);
								}, actor);
							}, actor);
