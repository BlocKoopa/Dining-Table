...
class ActorEvents_201 extends ActorScript
{
	...
	override public function init()
	{
		...
		addMouseOverActorListener(actor, function(mouseState:Int, list:Array<Dynamic>):Void
		{
			...
			{
				if((((Engine.engine.getGameAttribute("Answered16") : Bool) == false) && ((Engine.engine.getGameAttribute("Answered18") : Bool) == false)))
				{
					if(((Engine.engine.getGameAttribute("QuestionMenuLv1") : Bool) == false))
					{
						if(((Engine.engine.getGameAttribute("Correct_6") : Bool) == false))
						{
							actor.growTo(30/100, 25/100, 0, Easing.linear);
			...
			if(wrapper.enabled && -1 == mouseState)
			{
				if((((Engine.engine.getGameAttribute("Answered16") : Bool) == false) && ((Engine.engine.getGameAttribute("Answered18") : Bool) == false)))
				{
					if(((Engine.engine.getGameAttribute("QuestionMenuLv1") : Bool) == false))
					{
						if(((Engine.engine.getGameAttribute("Correct_6") : Bool) == false))
						{
							actor.growTo(25/100, 20/100, 0, Easing.linear);
							actor.clearFilters();
						}
					}
				}
			...
			if(wrapper.enabled)
			{
				if(((Engine.engine.getGameAttribute("QuestionMenuLv1") : Bool) == true))
				{
					if(((Engine.engine.getGameAttribute("Answered16") : Bool) == true))
					{
						...
						{
							actor.sendToBack();
							actor.moveTo(358, 58, 0.5, Easing.linear);
							actor.growTo(25/100, 20/100, 0.5, Easing.linear);
					...
					if(((Engine.engine.getGameAttribute("Answered17") : Bool) == true))
					{
						runLater(1000 * 2, function(timeTask:TimedTask):Void
						{
							actor.sendToBack();
					...
					if(((Engine.engine.getGameAttribute("Answered18") : Bool) == true))
					{
						runLater(1000 * 2, function(timeTask:TimedTask):Void
						{
							actor.sendToBack();
				...
				if((((Engine.engine.getGameAttribute("Answered16") : Bool) == false) && ((Engine.engine.getGameAttribute("Answered18") : Bool) == false)))
				{
					if(((Engine.engine.getGameAttribute("Correct_6") : Bool) == false))
					{
						if(((Engine.engine.getGameAttribute("QuestionMenuLv1") : Bool) == false))
						{
							...
							{
								createRecycledActor(getActorType(203), 800, 50, Script.FRONT);
								runLater(1000 * 0.5, function(timeTask:TimedTask):Void
								{
									createRecycledActor(getActorType(205), 800, 150, Script.FRONT);
									runLater(1000 * 0.5, function(timeTask:TimedTask):Void
									{
										createRecycledActor(getActorType(207), 800, 250, Script.FRONT);
										runLater(1000 * 0.5, function(timeTask:TimedTask):Void
										{
											createRecycledActor(getActorType(209), 800, 350, Script.FRONT);
										}, actor);
									}, actor);
								}, actor);
							}, actor);
